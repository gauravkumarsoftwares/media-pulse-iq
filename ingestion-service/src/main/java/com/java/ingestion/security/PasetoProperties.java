...
/**
 * Binds {@code platform.security.paseto.*} configuration.
 *
 * <p>When {@link #enabled} is true the service performs full in-service PASETO
 * v4.public verification (defense-in-depth) rather than blindly trusting the
 * gateway-injected {@code X-Tenant-Context} header.
 */
@Component
@Getter
@Setter
...
public class PasetoProperties {
    ...
    /** Enable in-service token verification. Disabled only for local dev. */
    ...
    /** Header carrying the token (gateway-forwarded). */
